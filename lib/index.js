...
exports['default'] = function () {
	...
	function linkedInputs(_ref2) {
		var input = _ref2.input;
		var state = _ref2.state;
		var output = _ref2.output;

		if (input.linkTo) {
			output.validate(_extends({ path: input.linkTo }, state.get(input.linkTo)));
		} else {
	...
	function setErrors(_ref3) {
		var _ref3$input = _ref3.input;
		var path = _ref3$input.path;
		var asyncError = _ref3$input.asyncError;
		var _ref3$input$errors = _ref3$input.errors;
		var errors = _ref3$input$errors === undefined ? {} : _ref3$input$errors;
		var _ref3$input$asyncValidation = _ref3$input.asyncValidation;
		var asyncValidation = _ref3$input$asyncValidation === undefined ? '' : _ref3$input$asyncValidation;
	...
	function checkIfValidationNeeded(_ref5) {
		var path = _ref5.input.path;
	...
	function chooseAsyncValidationPath(_ref6) {
	...
	return function (module) {
		...
		module.signals({
			...
			change: [setValue].concat(runValidationChain, validateLinked),
			focus: [],
			blur: [makeSurePathIsPresent, function (_ref10) {
				var input = _ref10.input;
				var state = _ref10.state;

				state.set([].concat(_toConsumableArray(input.path), ['visited']), true);
			}].concat(runValidationChain, validateLinked) });
		return {};
	};
};

function makeSurePathIsPresent(_ref11) {
